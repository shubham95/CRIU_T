...
int main(int argc, char *argv[], char *envp[])
{
	...
	printf("IN main\n");
	int time_log_fd = open("/home/uchiha/time_log_criu.txt",O_RDWR|O_APPEND|O_CREAT,0666);
	assert(time_log_fd >= 0);
	clock_gettime(CLOCK_REALTIME, &end_time);
	dprintf(time_log_fd,"Main funcion completion time : [%ld]\n",end_time.tv_sec - start_time.tv_sec);
	close(time_log_fd);

	//opening log file 
	//int time_log_fd = open("/home/connoisseur/time_log_criu.txt",O_RDWR|O_APPEND|O_CREAT);
	//assert(time_log_fd >= 0);
	//-------------
	int ret = -1;
